...
function getInfo(tableName){
    $.ajax({
        url: "http://150.136.104.157:8080/api/" + tableName + "/all",
        type: "GET",
        dataType: "JSON",
...
function displayAnswer(tableName, item){
    ...
    for(i = 0; i < item.length; ++i){
        if (tableName == "Category"){
            ...
            // Delete button.
            newTable += "<td> <button onclick='deleteInfo(" + '"' + tableName + '", ' + item[i].id + ")'>Borrar</button>";
        }
        else if (tableName == "Message"){
            newTable += "<tr>";
...
function saveInfo(tableName){
    ...
    $.ajax({
        url: "http://150.136.104.157:8080/api/" + tableName + "/save",
        type: "POST",
        data: dataToSend,
...
function updateInfo(tableName){
    ...
    let dataToSend = JSON.stringify(actualData);
    $.ajax({
        url: "http://150.136.104.157:8080/api/" + tableName + "/update",
        type: "PUT",
        data: dataToSend,
        contentType: "application/JSON; charset=utf-8",
        dataType: "JSON",
...
function deleteInfo(tableName, register_id){
    ...
    $.ajax({
        url: "http://150.136.104.157:8080/api/" + tableName + "/" + register_id.toString(),
        type: "DELETE",
        data: dataToSend,
